...
float EcalLaserDbService::getLaserCorrection(DetId const& xid, edm::Timestamp const& iTime) const {
  ...
  auto t_0_run = 1728955768; // Run 386924 -- currently hardcoded
  auto t_evt = iTime.unixTime();
  float t = t_evt - t_0_run; // time since run start expressed in seconds
  float transparency_norm = a*exp(-b*t) + (1-a)*exp(c*t); // analytic model used to fit normalized transparency data
  float transparency =  apdpnpair.p2/apdpnref * transparency_norm; 
  correctionFactor = 1.f / (std::pow(transparency, alpha));
  return correctionFactor;
 /* -- here the current correction method begins --
  // interpolation
